...
const simplifyChartData = (data, destinatedNumberOfPoints) => {
  ...
  if (allSegmentsPoints.length > destinatedNumberOfPoints) {
    ...
    const xMul = Math.floor(
      (allSegmentsPoints[allSegmentsPoints.length - 1].x - allSegmentsPoints[0].x) /
    ...
    for (let i = 1; i < destinatedNumberOfPoints - 1; i++) {
      ...
      newData.push({
        isImportant:
          (allSegmentsPoints[f].isImportant || allSegmentsPoints[f + 1].isImportant) &&
...
export default class Chart extends PureComponent {
  ...
  constructor(props) {
    ...
    allSegmentDividers = [];
    this.data = this.props.data.map(data => simplifyChartData(data, this.props.amountOfPathPoints));
    ...
    this.shouldReactToGestures = new Value(this.props.mode === 'gesture-managed' ? 1 : 0);
    ...
    this.chartAnimationValues = [new Value(1), new Value(0), new Value(0), new Value(0)];
    ...
    this.onTapGestureEvent = event([
      {
        nativeEvent: {
          state: state =>
            cond(
              or(eq(state, State.BEGAN), eq(state, State.END), eq(state, State.CANCELLED)),
              set(this.gestureState, state),
              delay(
                cond(
                  or(neq(state, State.FAILED), neq(this.panGestureState, State.ACTIVE)),
                  set(this.gestureState, state)
                ),
                100
              )
            ),
          x: x => cond(and(greaterThan(x, 0), lessThan(x, width)), set(this.touchX, x)),
        },
      },
    ]);
    ...
    [
      {
        nativeEvent: {
          x: x =>
            cond(and(neq(this.lastTouchX, x), greaterOrEq(x, 0), lessThan(x, width)), [
              set(this.touchX, x),
              set(this.lastTouchX, x),
            ]),
        },
      },
    ],
  ...
  reloadChart = async (currentInterval, isNewData = false) => {
    ...
    if (currentInterval !== this.currentInterval) {
      setTimeout(async () => {
        Animated.timing(this.chartAnimationValues[this.currentInterval], this._configDown).start();
        Animated.timing(this.chartAnimationValues[currentInterval], this._configUp).start();
  ...
  createAnimatedPath = () => {
    ...
    const segments = this.createSegmentColorsArray(allSegmentDividers, chartData);
    let splinePoints = [];
    for (let i = 0; i < chartData.length; i++) {
      if (chartData[i].points.length > 0) {
        ...
        const timestampLength =
          chartData[i].points[chartData[i].points.length - 1].x - chartData[i].points[0].x;
    ...
    let chartNode = (multiplyNode, index, chartIndex) =>
      multiply(
        ...
        add(
          ...
          multiply(
            this.value,
            sub(splinePoints[chartIndex][index].y2, splinePoints[chartIndex][index].y1)
          )
        )
      );
    ...
    for (let i = 0; i <= allSegmentDividers.length; i++) {
        ...
        ...splinePoints[0].flatMap(({ x }, index) => {
          if (i === 0) {
            ...
            if (i === allSegmentDividers.length) {
              ...
              if (index >= allSegmentDividers[i - 1] && index <= allSegmentDividers[i]) {
                return ['L', x, ' ', add(...allNodes(index))];
              }
            }
          }
      ...
      const colorMatrix = index => {
        return index < chartData.length - 1
          ? cond(
              ...
              Animated.color(...hexToRgb(chartData[index].colors[segments.colors[i][index]])),
              colorMatrix(index + 1)
            )
          : cond(
              eq(this.currentChart, index),
      ...
      const lineMatrix = index => {
        return index < chartData.length - 1
          ...
          : cond(eq(this.currentChart, index), chartData[index].lines[segments.colors[i][index]]);
      };
      ...
      returnPaths.push(
        <AnimatedPath
          ...
          strokeWidth={add(
            ...
            multiply(this.value, sub(this.props.stroke.simplified, this.props.stroke.detailed))
          )}
      ...
      if (chartData[element.index].startSeparatator[something[element.index]]) {
        returnPoints.push(
          <AnimatedCircle
            cx={element.x}
            cy={element.y}
            r={chartData[element.index].startSeparatator[something[element.index]].r}
            stroke={chartData[element.index].startSeparatator[something[element.index]].stroke}
            strokeWidth={
              chartData[element.index].startSeparatator[something[element.index]].strokeWidth
            }
            fill={chartData[element.index].startSeparatator[something[element.index]++].fill}
  ...
  render() {
    ...
    if (currentData.points.length > 0) {
      ...
      timePeriod = currentData.points[currentData.points.length - 1].x - currentData.points[0].x;

      maxValueDistance = this.checkValueBoundaries(
        ((maxValue.x - currentData.points[0].x) / timePeriod) * width - width / 2
      );
      minValueDistance = this.checkValueBoundaries(
        ((minValue.x - currentData.points[0].x) / timePeriod) * width - width / 2
      );
    }
        ...
        <GestureWrapper
          ...
          <Animated.View
            ...
            <Animated.View
              ...
              <Text style={{ transform: [{ translateX: maxValueDistance }] }}>
              ...
              <Animated.View
                style={[
                  ...
                  },
                  {
                    ...
                    transform: [
                      {
                        translateX: Animated.add(this.touchX, new Animated.Value(-1.5)),
                      },
                    ],
                  },
                ]}
            ...
            <Animated.View
              ...
              <Text style={{ transform: [{ translateX: minValueDistance }] }}>
          ...
          exec={block([
            ...
            cond(
              ...
              block([
                ...
                call([], () => {
                  this.props.onValueUpdate(currentData.points[currentData.points.length - 1].y);
                }),
              ]),
              onChange(
                this.touchX,
                call([this.touchX], ([x]) => {
                  ...
                  if (x < (width / (amountOfPathPoints + 20)) * 10) {
                    curX = 0;
                  } else if (x > width - (width / (amountOfPathPoints + 20)) * 10) {
                  ...
                  this.props.onValueUpdate(currentData.points[calculatedIndex].y);
                })
              )
            ),
